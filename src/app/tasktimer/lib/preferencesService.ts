...
export function createTaskTimerPreferencesService(options: PreferencesServiceOptions) {
  ...
  function buildSnapshot(state: PreferencesStateSnapshot): StoredPreferences {
    ...
    return {
      ...
      executiveFunctionEnabled: state.executiveFunctionEnabled !== false,
  ...
  function persistSnapshot(snapshot: StoredPreferences): void {
    ...
    safeWriteLocalStorage(
      storageKeys.EXECUTIVE_FUNCTION_ENABLED_KEY,
      snapshot.executiveFunctionEnabled !== false ? "true" : "false",
    );
    safeWriteLocalStorage(
    ...
    safeWriteLocalStorage(
      storageKeys.INTERACTION_HAPTICS_INTENSITY_KEY,
      normalizeInteractionHapticsIntensity(snapshot.interactionHapticsIntensity),
    );
    ...
    if (!uid) return;
    void options
      .syncOwnFriendshipProfile(uid, {
        currentRankId: snapshot.rewards?.currentRankId || null,
        totalXp: snapshot.rewards?.totalXp ?? null,
        completedTaskCount: snapshot.rewards?.completedSessions ?? null,
      })
      .catch(() => {});
  }
  ...
  function loadExecutiveFunctionEnabled(): boolean {
    const cloudValue = getStoredPreferencesWithoutDefaults()?.executiveFunctionEnabled;
    if (typeof cloudValue === "boolean") return cloudValue;
    const localValue = canUseLocalPreferenceFallback()
      ? parseStoredBoolean(safeReadLocalStorage(storageKeys.EXECUTIVE_FUNCTION_ENABLED_KEY))
      : null;
