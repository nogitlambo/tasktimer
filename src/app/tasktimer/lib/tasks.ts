...
export function formatMainTaskElapsedHtml(ms: number, isRunning = false): string {
  ...
  const minutes = Math.floor((totalSec % 3600) / 60);
  const seconds = totalSec % 60;
  const parts = [formatTwo(hours), formatTwo(minutes), formatTwo(seconds)];
  ...
  return `
      <span class="timePanel${panelStateClass}">
        <span class="${chunkClass(dimStates[0])}"><span class="timeBoxValue"><span class="timeBoxNum">${parts[0]}</span><span class="timeBoxUnit">H</span></span></span>
        <span class="${chunkClass(dimStates[1])}"><span class="timeBoxValue"><span class="timeBoxNum">${parts[1]}</span><span class="timeBoxUnit">M</span></span></span>
        <span class="${chunkClass(dimStates[2])}"><span class="timeBoxValue"><span class="timeBoxNum">${parts[2]}</span><span class="timeBoxUnit">S</span></span></span>
      </span>
    `;
}
