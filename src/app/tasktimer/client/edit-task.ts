...
export function normalizeRecurringScheduleFieldsForSave(
  ...
  if (sourceTask?.plannedStartOpenEnded && !normalizedByDay) {
    const nextByDay = buildWeeklyPlannedStartByDay(optimalProductivityDays || [], normalizedPlannedStartTime);
    if (nextByDay) {
      syncLegacyPlannedStartFields(task, nextByDay);
      return;
  ...
  const sourceByDay = sourceTask?.plannedStartOpenEnded ? null : normalizeTaskPlannedStartByDay(sourceTask?.plannedStartByDay);
  if (sourceByDay) {
...
export function createTaskTimerEditTask(ctx: TaskTimerEditTaskContext) {
  ...
  function syncEditTaskScheduleSummary(task?: Task | null) {
    const currentTask = task || getCurrentEditTask();
    const durationValue = Math.max(0, Number(els.editTaskDurationValueInput?.value || currentTask?.timeGoalValue || 0) || 0);
    const text =
      editTaskScheduleEnabled && durationValue > 0 && currentTask
        ? formatTaskScheduleSummary({
    ...
    if (els.editTaskScheduleSummaryText) {
      els.editTaskScheduleSummaryText.textContent = text;
    }
    els.editTaskScheduleSummary?.classList.toggle("isHidden", !text);
  }
  ...
  function finalizeEditSave(sourceTask: Task, t: Task) {
    ...
    const overlap = findScheduleOverlap(ctx.getTasks(), t, { excludeTaskId: sourceTaskId });
    if (overlap) {
      ctx.syncEditSaveAvailability(t);
      openEditScheduleConflictModal(sourceTask, t, overlap);
      return false;
    }
  ...
  function openEditScheduleConflictModal(
    sourceTask: Task,
    draftTask: Task,
    overlap: NonNullable<ReturnType<typeof findScheduleOverlap>>
  ) {
    const conflictingTask = overlap.task || null;
    const sourceTaskId = String(sourceTask.id || "");
    if (!conflictingTask || overlap.candidateStartMinutes == null || overlap.conflictingStartMinutes == null) {
    ...
    const suggestion = findNextAvailableScheduleSlot(ctx.getTasks(), draftTask, { excludeTaskId: sourceTaskId });
    ...
    const handleMove = () => {
      ...
      suggestion.days.forEach((day) => {
        setTaskScheduledTimeForDay(draftTask, day, nextTime);
      });
      draftTask.plannedStartTime = nextTime;
      syncEditPlannedStartSelectors(draftTask);
  ...
  function closeEdit(saveChanges: boolean) {
    ...
    if (saveChanges && t && sourceTask) {
      ...
      if (!persistScheduledEditSave(sourceTask, t)) return;
      didSave = true;
    }
