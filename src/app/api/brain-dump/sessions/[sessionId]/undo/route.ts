...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    return withAuthenticatedApiCors(req, NextResponse.json({ ok: true, undo }));
  } catch (error) {
    if (hasStatus(error)) {
      return withAuthenticatedApiCors(
        req,
        NextResponse.json(
          { error: errorMessage(error, "Could not undo Brain Dump tasks."), code: errorCode(error) },
          { status: errorStatus(error) }
        )
      );
    }
    console.error("[api/brain-dump/sessions/sessionId/undo] Request failed", {
      name: error instanceof Error ? error.name : "unknown",
      code: errorCode(error),
    });
    return withAuthenticatedApiCors(
      req,
      NextResponse.json({ error: "Could not undo Brain Dump tasks.", code: "internal" }, { status: 500 })
    );
  }
}
