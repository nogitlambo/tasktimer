...
import {
  BrainDumpImageInterpretationError,
  BrainDumpInputError,
  BrainDumpProviderValidationError,
  processImageBrainDump,
...
export async function POST(req: Request) {
  try {
    ...
    const session = await processImageBrainDump({
      uid,
    ...
    if (error instanceof Error && hasStatus(error) && errorCode(error).startsWith("auth/")) {
      return withAuthenticatedApiCors(
        req,
        NextResponse.json(
          { error: errorMessage(error, "Could not process Brain Dump image."), code: errorCode(error) },
          { status: errorStatus(error) }
        )
      );
    }
    if (
      error instanceof BrainDumpInputError ||
      error instanceof BrainDumpImageInterpretationError ||
      error instanceof BrainDumpProviderValidationError ||
      hasStatus(error)
    ) {
      return withAuthenticatedApiCors(
        req,
    ...
    console.error("[api/brain-dump/images] Request failed", {
      name: error instanceof Error ? error.name : "unknown",
      code: errorCode(error),
    });
    return withAuthenticatedApiCors(
      req,
      NextResponse.json({ error: "Could not process Brain Dump image.", code: "internal" }, { status: 500 })
    );
  }
}
