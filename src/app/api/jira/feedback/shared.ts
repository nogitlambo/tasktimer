...
export function parseJiraIssueKeyFromBrowseUrl(urlRaw: unknown) {
...
export async function syncJiraIssueVote(input: {
  ...
  const updateResponse = await fetch(`${input.jiraBaseUrl}/rest/api/3/issue/${input.jiraIssueKey}`, {
    method: "PUT",
    headers: getJiraAuthHeaders(),
    body: JSON.stringify({
      fields: {
...
export async function fetchJiraIssueStatuses(input: {
  jiraBaseUrl: string;
  ...
  const lookupResults = await Promise.allSettled(
    jiraIssueKeys.map(async (jiraIssueKey) => {
      const response = await fetch(`${input.jiraBaseUrl}/rest/api/3/issue/${encodeURIComponent(jiraIssueKey)}?fields=status`, {
        method: "GET",
        headers: getJiraAuthHeaders(),
        cache: "no-store",
      });
      ...
      return {
        requestedKey: jiraIssueKey,
        responseKey: asString(payload?.key, 120) || jiraIssueKey,
        name: asString(payload?.fields?.status?.name, 120),
        category: asString(payload?.fields?.status?.statusCategory?.key, 120).toLowerCase(),
        categoryName: asString(payload?.fields?.status?.statusCategory?.name, 120).toLowerCase(),
      };
    })
  );
  ...
  lookupResults.forEach((result) => {
    if (result.status !== "fulfilled") {
      console.warn("[api/jira/feedback] Jira status lookup failed for issue", {
        error: describeError(result.reason),
      });
      return;
    }
    const issue = result.value;
    const key = asString(issue.responseKey || issue.requestedKey, 120);
    if (!key) return;
    statuses[key] = {
      name: issue.name,
