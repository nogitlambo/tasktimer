...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    await enforceUidRateLimit({ namespace: "recovery/dismiss", uid, windowMs: 60_000, maxEvents: 12, code: "recovery/rate-limited", message: "Please wait before dismissing another Recovery Mode session." });
    ...
    if (existing.status !== "ACTIVE" && existing.status !== "PARTIALLY_APPLIED") return withAuthenticatedApiCors(req, NextResponse.json({ error: "This Recovery Mode session cannot be dismissed.", code: "recovery/not-active" }, { status: 409 }));
    const nowMs = Date.now();
    const dismissed = await repository.dismissSession(uid, recoveryId, nowMs);
    if (!dismissed || dismissed.status !== "DISMISSED") return withAuthenticatedApiCors(req, NextResponse.json({ error: "This Recovery Mode session is no longer active.", code: "recovery/not-active" }, { status: 409 }));
    await createFirestoreRecoveryEligibilityRepository(db).recordDismissal(uid, nowMs);
    return withAuthenticatedApiCors(req, NextResponse.json({ ok: true, session: dismissed }));
  } catch (error) {
    ...
    console.error("[api/executive-function/recovery/dismiss] Request failed", { code, status: safeStatus });
    return withAuthenticatedApiCors(req, NextResponse.json({ error: "TaskLaunch could not dismiss Recovery Mode right now.", code: "recovery/internal" }, { status: 500 }));
  }
}
