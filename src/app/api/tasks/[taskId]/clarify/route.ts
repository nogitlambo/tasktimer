...
const GENERATION_FAILURE_MESSAGE = "TaskLaunch could not prepare a reliable breakdown. Your task has not been changed.";
...
function recommendationReviewPayload(recommendation: TaskClarificationRecommendation) {
  return Object.fromEntries(
    Object.entries(recommendation).filter(([key]) => key !== "userId" && key !== "auditExpiresAt")
  );
}
...
export async function POST(req: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!taskId) {
      return withAuthenticatedApiCors(req, NextResponse.json({ error: "Task not found.", code: "task/not-found" }, { status: 404 }));
    }
    ...
    const response = await generateValidatedTaskClarification({
      taskId: task.taskId,
      title: task.title,
    ...
    return withAuthenticatedApiCors(
      req,
      NextResponse.json({ ok: true, recommendation: recommendationReviewPayload(recommendation) })
    );
  } catch (error) {
    if (error instanceof TaskClarificationInputError) {
      return withAuthenticatedApiCors(req, NextResponse.json({ error: error.message, code: error.code }, { status: error.status }));
    }
    if (error instanceof ApiRateLimitError) {
      return withAuthenticatedApiCors(req, NextResponse.json({ error: error.message, code: error.code }, { status: error.status }));
    }
    if (error instanceof TaskClarificationProviderUnavailableError || error instanceof TaskClarificationProviderError) {
      return withAuthenticatedApiCors(
        req,
        NextResponse.json({ error: GENERATION_FAILURE_MESSAGE, code: error.code }, { status: error.status })
      );
    }
    if (error instanceof ZodError || errorCode(error) === "task-clarification/provider-invalid") {
      return withAuthenticatedApiCors(
        req,
        NextResponse.json({ error: GENERATION_FAILURE_MESSAGE, code: "task-clarification/provider-invalid" }, { status: 502 })
      );
    }
    if (error instanceof Error && "status" in error) {
      return withAuthenticatedApiCors(req, createApiAuthErrorResponse(error, GENERATION_FAILURE_MESSAGE));
    }
    console.error("[api/task-clarification] Request failed", {
    ...
    return withAuthenticatedApiCors(
      req,
      NextResponse.json({ error: GENERATION_FAILURE_MESSAGE, code: "internal" }, { status: 500 })
    );
  }
}
